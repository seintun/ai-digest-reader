...
|---|---|---|---|---|---|---|---|---|
| T1 | setup | coordination | coordinator | in_progress | none | worktree + plan/tracker created | `git status` | this file + plan file |
| T2 | phase0 | verification | coordinator | pending | T1 | baseline test/build timings captured | `.venv/bin/python -m pytest && npm run check && npm run build` | baseline report |
...
| T6 | phase1 | security | coordinator | pending | T2,T5 | remove XSS/url risks + strong validation | `.venv/bin/python -m pytest && npm run check` | test and diff evidence |
