...
`.***REDACTED_RUBYGEMS_KEY***.yml` runs on schedule: 7:00 AM and 6:00 PM UTC.
...
```bash
.***REDACTED_RUBYGEMS_KEY***.sh   # Installs 7am + 6pm cron jobs
```
Logs: `logs/digest.log`

## Pipeline Steps
1. `python digest.py` — fetch Reddit + HN + RSS, generate AI summary, write `***REDACTED_RUBYGEMS_KEY***.json`
...
```bash
***REDACTED_RUBYGEMS_KEY*** .***REDACTED_RUBYGEMS_KEY***.sh
```
