...
- `analyzer.py` retains ***REDACTED_RUBYGEMS_KEY*** behavior with a stricter prompt for a second attempt.

## Adding a New Model
Set `OPENROUTER_API_KEY` and change the model in `analyzer.py` and `ranker.py` OpenRouter calls:
```python
model="moonshotai/kimi-k2.6"  # or "x-ai/grok-4-0131", "***REDACTED_RUBYGEMS_KEY***.5", etc.
```
