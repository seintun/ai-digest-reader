...
**archive.today** (`***REDACTED_RUBYGEMS_KEY***` in `scraper.py`) caches full rendered HTML of pages, including paywalled content. No API key required. Returns archived content for sites that block both direct requests and Jina (CNBC, WSJ, Bloomberg, Fortune, CBS News). Timeout is 10s, configurable via `***REDACTED_RUBYGEMS_KEY***`.
