...
def ***REDACTED_RUBYGEMS_KEY***(candidates: List[Tuple[str, str]]) -> Tuple[Optional[Dict[str, int]], Dict[str, float | int]]:
...
def _estimate_quality_cost_usd(candidates: List[Tuple[str, str]], workers: int) -> float:
    ...
    return float(usage_to_dict(***REDACTED_RUBYGEMS_KEY***, ***REDACTED_RUBYGEMS_KEY***).get("cost_usd", 0.0))


def ***REDACTED_RUBYGEMS_KEY***(posts: List[Dict], scraped_content: Dict[str, str]) -> Tuple[Optional[Dict[str, int]], Dict[str, float | int]]:
    api_key = os.environ.get("OPENROUTER_API_KEY")
    if not api_key:
        ...
        usage.update({
            ...
            "ai_batches": 0,
            "***REDACTED_RUBYGEMS_KEY***": "missing_api_key",
        })
        return None, usage
