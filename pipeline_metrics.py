...
def render_dashboard(metrics: Dict) -> str:
    ...
    lines = [
        ...
        f"- Session Model Cost / Estimate (USD): `${cost.get('session_model_usd', 0):.6f}`",
        f"- Estimated Tokens: `{cost.get('estimated_tokens', 0)}`",
        f"- Cost Source: `{cost.get('pricing_source', 'unknown')}`",
